...
async function loadData(selectedId = null) { loading.value = true; try { const data = await getDocumentWorkflows(); workflows.value = (data.workflows ?? []).map(normalize); users.value = data.users ?? []; if (selectedId) { const found = workflows.value.find(item => item.id === selectedId); if (found) selectWorkflow(found) } } catch (e) { setMessage('error', errorMessage(e, 'Не удалось загрузить документы.')) } finally { loading.value = false } }
