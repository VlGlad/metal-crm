...
final class DocumentWorkflowFileStorage
{
    private const MAX_FILE_SIZE = 20 * 1024 * 1024;

    public function __construct(private readonly string $uploadDirectory) {}

    public function store(UploadedFile $file, int $workflowId): array
    {
        if (!$file->isValid()) throw new \RuntimeException('Не удалось загрузить файл.');
        $size = $file->getSize();
        if (!is_int($size) || $size <= 0) throw new \RuntimeException('Нельзя загрузить пустой файл.');
        if ($size > self::MAX_FILE_SIZE) throw new \RuntimeException('Размер одного файла не должен превышать 20 МБ.');
        $originalName = trim(basename(str_replace('\\', '/', $file->getClientOriginalName())));
        if ($originalName === '' || mb_strlen($originalName) > 255) throw new \RuntimeException('Некорректное имя файла.');
        $extension = mb_strtolower($file->getClientOriginalExtension()) ?: 'bin';
        $relativeDirectory = (string) $workflowId;
    ...
    public function path(string $storedName): string { return rtrim($this->uploadDirectory, '/').'/'.ltrim($storedName, '/'); }
